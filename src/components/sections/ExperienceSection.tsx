...
const ExperienceSection: React.FC = () => {
  ...
  const timelineData = t('experience.items', { returnObjects: true }) as TimelineItem[];

  const volleyItems = timelineData.filter((item) =>
    item.title.toLowerCase().includes('volley')
  );
  const educationItems = timelineData.filter((item) => item.type === 'education');
  const professionalItems = timelineData.filter(
    (item) => item.type === 'experience' && !item.title.toLowerCase().includes('volley')
  );
    ...
    <section id="experience" className="bg-black py-20 px-4 text-white">
      <div className="w-[90%] max-w-none mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-6">
          ...
          <div>
            <h3 className="text-2xl font-semibold text-indigo-300 mb-6">
              {t('experience.sections.professional')}
            </h3>
          ...
          <div>
            <h3 className="text-2xl font-semibold text-indigo-300 mb-6">
              {t('experience.sections.volley')}
            </h3>
            {renderTimeline(volleyItems)}
          </div>
        </div>
      </div>
    </section>
